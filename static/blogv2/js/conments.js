...
function showmsg(str) {
    getid('modal-body').innerText = "";
    getid('modal-body').innerText = str;
}

// 发送留言
function sendstagmessage() {
    var textaear = getid('textarea');
    var code = getid('code');
    if (!textaear.value | !code.value){
        showmsg("请将数据补充完整");
        $('#myModal').modal('show');
        return ;
    }
    if(textaear.value.length > 500){
        showmsg("留言字数要求小于500");
        $('#myModal').modal('show');
        return ;
    }
    var xmlhttp = GetXmlHttp();
    xmlhttp.open("POST", staymessageurl, true);
    xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
    var data = "textarea=" + textaear.value + "&code=" + code.value + "&csrfmiddlewaretoken=" + form_stay.csrfmiddlewaretoken.value;
    xmlhttp.send(data);
    xmlhttp.onreadystatechange = function () {
